...
//     const addresses = await factory.methods.getDeplyedCampaigns().call();
...
describe("Campaigns", () => {
    ...
    it("allows manager to make payment requests", async() => {
        await campaign.methods.createRequest("Samosa", 1, accounts[1]).send({
            from: accounts[0],
            gas: 1000000
        });

        const request = await campaign.methods.requests(0).call();
        assert.strictEqual("Samosa", request.description);
